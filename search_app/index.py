...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        postings_list = self.index_file.read(len_in_bytes_of_postings)
        tf_list = self.index_file.read(len_in_bytes_of_tf)
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        start_pos = self.index_file.seek(0,2)
        num_postings = len(postings_list)
        length_postings_list_in_byte = len(encoded_postings_list)
        length_tf_list_in_byte = len(encoded_tf_list)

        self.postings_dict[term] = (start_pos, num_postings, length_postings_list_in_byte, length_tf_list_in_byte)

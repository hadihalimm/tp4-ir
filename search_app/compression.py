...
class VBEPostings:
    ...
    def vb_decode(encoded_bytestream):
        ...
        """
        # TODO
        encoded_list = list(encoded_bytestream)
        numbers = []
        n = 0

        for i in range(len(encoded_list)):
            if (encoded_list[i] < 128):
                n = 128*n + encoded_list[i]
            else:
                n = 128*n + (encoded_list[i] - 128)
                numbers.append(n)
                n = 0 
    ...
    def decode(encoded_postings_list):
        ...
        """
        # TODO
        decoded_list = VBEPostings.vb_decode(encoded_postings_list)
        postings_list = []
        postings_list.append(decoded_list[0])

        for i in range(1, len(decoded_list)):
            docId = postings_list[i-1] + decoded_list[i]
